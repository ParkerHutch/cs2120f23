...
/-!
Function that goes from α to β to γ
...
#reduce is_even_len "Hello!"

def comp { α β γ : Type } : (β → γ) → (α → β) → (α → γ)
| g, f => fun x => g (f x)

def is_even_len' : String → Bool := comp is_even String.length

def is_even_len'' := is_even ∘ String.length

-- def apply2 {α β: Type } : (α → β)
-- | a => a (

-- need to take in argument of type Type (function) and return a function

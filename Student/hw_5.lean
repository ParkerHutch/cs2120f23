...
#reduce box_map Nat.succ (Box.contents 0)
...
Here:
the map function above expects the container object it's being applied to to
only contain one child object. The tree we defined can have multiple children
and so it won't work with this map function.
-/
...
infix:50 "<$>"  => do_map
#eval Nat.succ <$> [1,2,3] -- #eval do_map list_functor Nat.succ [1,2,3]  -- [2, 3, 4]
#eval (λ s => s ++ "!") <$> ["Hi", "Yo"]

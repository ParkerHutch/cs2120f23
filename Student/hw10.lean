...
def len_cons : (List Nat) → Nat
| List.nil => len_nil
| List.cons h t => 1 + len_cons t

def list_step : Nat → List Nat → Nat → Nat
| α, lst, n => n + 1
...
def tree_cons : (Tree Nat) → Nat
| Tree.empty => tree_nil
| Tree.node val l r => tree_cons l + tree_cons r

def tree_step : Nat → Tree Nat → Tree Nat → Nat → Nat → Nat
| a, l, r, x, n => n + 1

-- 3. compose them using Tree.rec and apply to a_tree, expecting 3
#reduce (List.rec 0 list_step : List Nat → Nat) [1,2,3]
#reduce (Tree.rec 1 tree_step : Tree Nat → Nat) a_tree
